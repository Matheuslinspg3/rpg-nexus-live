...
export function ShieldWorkspace({ campaignCode, role, characters, scene, rolls, rollBusy, onRoll, onReveal, onOpenCharacter, onGoTo }: {
  ...
  const resizeModule = (id: ModuleId) => {
    ...
    void saveLayout({ ...layout, spans: { ...layout.spans, [id]: next } });
  };
  ...
  const renderContent = (id: ModuleId) => {
    ...
    if (id === "youtube") return <ShieldYoutube value={layout.youtubeUrl} onChange={(youtubeUrl) => void saveLayout({ ...layout, youtubeUrl })} />;
    return <ShieldPdf value={layout.pdfUrl} onChange={(pdfUrl) => void saveLayout({ ...layout, pdfUrl })} />;
  };
